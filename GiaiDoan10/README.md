...
# README - Giai đoạn 9 (Hoàn thiện luồng thanh toán và chuẩn hóa mã hiển thị phiếu khám)

## 66. Tổng quan giai đoạn 9

Giai đoạn 9 tập trung vào mục tiêu khép kín chuỗi nghiệp vụ sau khám bệnh: từ phiếu khám chưa thanh toán đến tạo hóa đơn và xác nhận thanh toán ngay trên module thu ngân.

Trọng tâm chính của giai đoạn này:

- Triển khai logic thật cho `FrmThuNgan` thay cho trạng thái khung giao diện.
- Bổ sung cột mã hiển thị (`MaHienThi`) cho phiếu khám để hỗ trợ tìm kiếm và hiển thị nghiệp vụ thân thiện.
- Đồng bộ lại mô hình chi tiết đơn thuốc theo hướng gọn hơn (bỏ cột `CachDung` trong bảng chi tiết).
- Nâng mức liên thông điều hướng từ trang chủ tới các form thanh toán và thông tin khách hàng.

Nếu giai đoạn 8 là bước hoàn thiện CRUD danh mục, thì giai đoạn 9 là bước đẩy hệ thống tiến gần hơn tới vận hành thực tế ở khâu thu phí.

---

## 67. Các form và luồng nghiệp vụ nổi bật ở giai đoạn 9

### 67.1. `FrmThuNgan` (nâng từ khung UI lên xử lý nghiệp vụ thực)

Các điểm đã triển khai:

- Nạp danh sách phiếu khám chưa thanh toán dựa trên điều kiện `NOT EXISTS` hóa đơn.
- Hỗ trợ tìm kiếm theo mã phiếu hiển thị hoặc tên bệnh nhân.
- Khi chọn phiếu khám, hệ thống tải chi tiết dịch vụ và thuốc theo từng dòng.
- Tính tổng tiền từ 2 nguồn chi phí (dịch vụ + thuốc) và hiển thị ngay trên form.
- Xác nhận thanh toán bằng cách `INSERT` hóa đơn vào bảng `HOADON`.
- Sau khi thanh toán, cho phép nhắc in hóa đơn (mức thông báo) và làm sạch trạng thái UI.
- Tự động reload danh sách phiếu chưa thanh toán để loại bỏ phiếu vừa xử lý.

### 67.2. `FrmTrangChu` (điều hướng được đồng bộ tốt hơn)

Các điểm đã thể hiện rõ trong mã:

- Có hàm mở module thu ngân và gắn đầy đủ cho cả menu lẫn toolstrip (`mnuThanhToan`, `tsbThanhToan`).
- Có hàm mở module khách hàng và gắn đầy đủ cho cả menu lẫn toolstrip (`mnuThongTinKhachHang`, `tsbDatLich`).
- Giữ cấu trúc phân quyền theo vai trò, đảm bảo vai trò nhân viên/admin truy cập được luồng thanh toán.

---

## 68. Cập nhật dữ liệu và migration ở giai đoạn 9

### 68.1. Migration `ThemCotMaHienThi`

- Thêm cột `MaHienThi` vào bảng `PHIEUKHAM` (`nvarchar(20)`, `NOT NULL`, mặc định chuỗi rỗng).
- Tạo nền để mã nghiệp vụ hiển thị rõ ràng hơn (ví dụ dạng `MPK...`) thay vì chỉ dùng số ID nội bộ.

### 68.2. Migration `XoaCotCachDung`

- Bỏ cột `CachDung` khỏi bảng `CHITIET_DONTHUOC`.
- Đồng bộ với entity `ChiTietDonThuoc` hiện tại chỉ còn các trường chính: mã phiếu khám, mã thuốc, số lượng.

### 68.3. Migration `CapNhat_DonGia_MaDV`

- Migration đã được tạo nhưng phần `Up/Down` đang để trống.
- Đây là dấu hiệu kỹ thuật cần lưu ý vì dễ tạo hiểu nhầm là đã thay đổi schema trong khi thực tế chưa có thao tác DDL nào.

---

## 69. Trạng thái build và chất lượng mã

Kết quả build tại giai đoạn 9:

- Dự án build thành công.
- Có 86 warnings, chủ yếu xoay quanh:
  - Nullability (`CS8618`, `CS8600`, `CS8602`, `CS8601`, `CS8605`, `CS8622`).
  - Cảnh báo field chưa được gán (`CS0649`).

Nhận định:

- Hệ thống đã đạt mức chạy được về mặt biên dịch và triển khai chức năng.
- Tuy nhiên nợ kỹ thuật về null-safety vẫn cao; nếu không xử lý dần sẽ tạo rủi ro runtime và giảm tốc độ bảo trì ở các giai đoạn sau.

---

## 70. Lỗi sai và rủi ro kỹ thuật có thể gặp ở giai đoạn 9

### 70.1. Rủi ro bảo mật từ chuỗi kết nối hard-code trong form

- `FrmThuNgan` đang chứa trực tiếp chuỗi kết nối kèm thông tin tài khoản SQL trong mã nguồn.
- Đây là rủi ro lớn về bảo mật và vận hành (lộ credential, khó đổi môi trường, khó xoay vòng mật khẩu).

### 70.2. Rủi ro xử lý mã hiển thị phiếu khám bằng thao tác chuỗi

- Luồng hiện tại cắt tiền tố `MPK` để lấy ID số nội bộ.
- Nếu format mã hiển thị thay đổi hoặc chứa ký tự không đúng chuẩn, có thể phát sinh lỗi parse/truy vấn sai.

### 70.3. Rủi ro mất dữ liệu mô tả cách dùng thuốc sau migration

- Việc bỏ cột `CachDung` giúp mô hình gọn hơn nhưng đồng thời loại bỏ một thông tin nghiệp vụ có thể hữu ích cho điều trị.
- Cần cân nhắc lưu thông tin này ở thực thể phù hợp khác nếu nghiệp vụ yêu cầu theo dõi hướng dẫn dùng thuốc.

### 70.4. Rủi ro khi có migration rỗng tồn tại lâu dài

- Migration `CapNhat_DonGia_MaDV` không có thao tác thực thi.
- Nếu không quản lý tốt lịch sử migration, đội phát triển có thể hiểu sai trạng thái DB giữa các môi trường.

### 70.5. Rủi ro ổn định do warning nullability tồn đọng

- Các điểm cảnh báo dereference null trong form nghiệp vụ (bao gồm thu ngân) có thể trở thành lỗi runtime khi dữ liệu đầu vào không như kỳ vọng.

---

## 71. Khó khăn thực tế và kinh nghiệm rút ra ở giai đoạn 9

### 71.1. Khó khăn khi ghép dữ liệu tổng hợp từ nhiều bảng

- Module thanh toán phải tổng hợp dữ liệu từ phiếu khám, lịch khám, khách hàng, bác sĩ, chi tiết dịch vụ và chi tiết thuốc.
- Kinh nghiệm: cần chốt rõ hợp đồng dữ liệu hiển thị (view model/cột grid) trước khi code SQL để tránh lệch cột.

### 71.2. Khó khăn khi cân bằng tốc độ hoàn thiện chức năng và chất lượng mã

- Việc ưu tiên hoàn thành luồng thanh toán giúp đạt tiến độ nghiệp vụ, nhưng kéo theo warning kỹ thuật và một số xử lý còn thủ công.
- Kinh nghiệm: sau mỗi mốc nghiệp vụ lớn nên có đợt hardening riêng (nullability, config hóa connection string, chuẩn hóa xử lý lỗi).

### 71.3. Khó khăn trong chuẩn hóa mã nghiệp vụ hiển thị

- Chuyển từ mã ID nội bộ sang mã hiển thị thân thiện là bước đúng, nhưng cần chuẩn hóa quy tắc sinh mã và tách biệt rõ giữa display code và primary key.

---

## 72. Đánh giá kết quả giai đoạn 9

**Đã hoàn thành tốt:**

- Hoàn thiện được luồng thanh toán thực tế trên `FrmThuNgan` (lọc phiếu chưa thu, xem chi tiết, tính tổng, tạo hóa đơn).
- Đồng bộ điều hướng tại `FrmTrangChu` cho các luồng thanh toán và khách hàng.
- Bổ sung thay đổi schema có ý nghĩa nghiệp vụ (`MaHienThi`) để cải thiện trải nghiệm vận hành.
- Duy trì trạng thái build thành công.

**Cần cải thiện tiếp:**

- Đưa chuỗi kết nối ra `App.config`/biến môi trường để đảm bảo an toàn bảo mật.
- Xử lý dần warning nullability để giảm rủi ro runtime.
- Rà soát và dọn migration rỗng để lịch sử schema rõ ràng, nhất quán.

---

## 73. Kết luận giai đoạn 9

Giai đoạn 9 đánh dấu bước chuyển quan trọng từ "quản lý dữ liệu" sang "khép kín giao dịch" trong hệ thống phòng khám: module thu ngân đã có thể xử lý thanh toán thực tế dựa trên dữ liệu dịch vụ và thuốc, đồng thời trang chủ đã liên thông tốt hơn tới các luồng tác nghiệp cần thiết. Dù vậy, để tiến tới mức sẵn sàng triển khai ổn định, dự án cần ưu tiên xử lý nợ kỹ thuật nullability, chuẩn hóa cấu hình kết nối và làm sạch lịch sử migration trong các giai đoạn kế tiếp.
...
# README - Giai đoạn 10 (Hoàn thiện điều hướng hệ thống và tinh chỉnh trải nghiệm người dùng)

## 74. Tổng quan giai đoạn 10

Giai đoạn 10 tập trung vào việc hoàn thiện lớp điều hướng ở màn hình chính và tinh chỉnh trải nghiệm hiển thị cho người dùng cuối, đặc biệt ở module khách hàng. Nếu giai đoạn 9 ưu tiên khép kín luồng thanh toán, thì giai đoạn 10 ưu tiên tính nhất quán thao tác giữa menu và toolbar, đồng thời giảm các điểm gây hiểu nhầm trong giao diện.

Trọng tâm chính:

- Chuẩn hóa lại cấu trúc menu ở `FrmTrangChu` theo phạm vi chức năng đang dùng thực tế.
- Bổ sung đầy đủ event handler cho các mục menu quản lý (Thuốc, Dịch vụ, Khách hàng, Admin).
- Tinh chỉnh nhãn mặc định và phối màu nền ở `FrmKhachHang` để giao diện rõ trạng thái hơn.
- Giữ ổn định trạng thái build để chuẩn bị cho giai đoạn hardening kỹ thuật sau cùng.

---

## 75. Các thay đổi nổi bật ở giai đoạn 10

### 75.1. Hoàn thiện điều hướng menu trong `FrmTrangChu`

Các điểm mới đã có trong mã:

- Bổ sung click handler cho menu `mnuQuanLyThuoc` để mở đúng `FrmQuanLyThuoc` (đồng bộ với nút toolstrip `tsbThuoc`).
- Bổ sung click handler cho menu `mnuQuanLyDichVu` để mở đúng `FrmDichVu` (đồng bộ với `tsbDichVu`).
- Bổ sung click handler cho menu `mnuQuanLyKhachHang` để mở `FrmKhachHang` theo user hiện tại.
- Bổ sung click handler cho menu `mnuAdmin` để mở `FrmAdmin`.

Ý nghĩa:

- Tránh tình trạng cùng chức năng nhưng chỉ chạy ở toolbar, còn menu không phản hồi.
- Tăng độ nhất quán hành vi giữa các cách thao tác khác nhau của người dùng.

### 75.2. Tối giản lại cấu trúc menu không còn phù hợp

Ở Designer của `FrmTrangChu`, giai đoạn 10 đã loại bớt một số mục không còn được dùng trong điều hướng hiện tại:

- Không còn menu `mnuThoat` trong nhóm hệ thống.
- Không còn hai mục `mnuQuanLyBacSi` và `mnuQuanLyNhanVien` trong nhóm quản lý.

Điều này giúp menu rõ ràng hơn, bám sát các luồng đã có form/hành vi cụ thể.

### 75.3. Tinh chỉnh giao diện `FrmKhachHang`

Các thay đổi nổi bật:

- Đổi nhãn mặc định nhận diện bệnh nhân:
  - `Bệnh Nhân: Chưa xác định`
  - `Mã BN: chưa xác định`
- Thêm màu nền cho vùng hồ sơ cá nhân:
  - Panel thông tin cá nhân dùng `WhiteSmoke`.
  - Panel đổi mật khẩu dùng `OldLace`.

Giá trị mang lại:

- Trạng thái ban đầu của màn hình rõ ràng hơn, giảm nhầm lẫn với dữ liệu thật.
- Bố cục tab hồ sơ có phân tách thị giác tốt hơn.

---

## 76. Trạng thái dữ liệu và migration ở giai đoạn 10

Kết quả rà soát thư mục `Migrations` cho thấy:

- Không phát sinh migration mới so với giai đoạn 9.
- Bộ migration hiện tại vẫn dừng ở các mốc đã có: thêm `MaHienThi`, bỏ `CachDung`, và migration rỗng `CapNhat_DonGia_MaDV`.

Nhận định:

- Giai đoạn 10 chủ yếu là tối ưu UI/điều hướng, không thay đổi schema dữ liệu.

---

## 77. Trạng thái build và chất lượng mã

Kết quả build giai đoạn 10:

- Build thành công.
- Còn 86 warnings, phần lớn thuộc nhóm nullability (`CS8618`, `CS8600`, `CS8602`, `CS8601`, `CS8605`, `CS8622`) và field chưa gán (`CS0649`).

Đánh giá:

- Mốc giai đoạn 10 đạt mục tiêu ổn định chức năng điều hướng/UI.
- Tuy nhiên mức cảnh báo chưa giảm so với giai đoạn 9, cho thấy nhu cầu hardening kỹ thuật vẫn rất rõ ràng.

---

## 78. Lỗi sai và rủi ro kỹ thuật có thể gặp ở giai đoạn 10

### 78.1. Rủi ro tối giản menu nhưng thiếu truyền thông nghiệp vụ

- Việc ẩn/bỏ một số mục quản lý giúp UI gọn hơn, nhưng nếu không có ghi chú vận hành có thể gây hiểu nhầm về quyền hoặc nơi thao tác tương ứng.

### 78.2. Rủi ro lệch hành vi khi mở form từ nhiều entry-point

- Khi cùng một form mở được từ cả menu và toolbar, cần đảm bảo cùng luồng truyền tham số và phân quyền để tránh sai khác hành vi.

### 78.3. Rủi ro nợ kỹ thuật kéo dài do warning không giảm

- Dù giai đoạn 10 đã cải thiện UX, warning nullability vẫn ở mức cao và có thể dẫn tới lỗi runtime trong các luồng dữ liệu không chuẩn.

### 78.4. Rủi ro bảo mật tồn đọng từ giai đoạn trước

- Vấn đề connection string hard-code trong module thu ngân vẫn chưa được xử lý trong giai đoạn này.

---

## 79. Khó khăn và kinh nghiệm rút ra ở giai đoạn 10

### 79.1. Khó khăn khi đảm bảo nhất quán giữa Designer và code-behind

- Điều hướng menu trong WinForms phụ thuộc chặt vào việc gắn sự kiện trong Designer.
- Kinh nghiệm: mỗi lần chỉnh menu cần rà soát lại toàn bộ event map để tránh mục hiện nhưng không chạy.

### 79.2. Khó khăn khi cải tiến UX nhưng vẫn giữ ổn định luồng cũ

- Các tinh chỉnh UI nhỏ (label, màu nền, bố cục) phải đi cùng kiểm thử thao tác để không làm vỡ các luồng đã ổn định từ giai đoạn trước.

### 79.3. Khó khăn trong cân bằng giữa hoàn thiện tính năng và trả nợ kỹ thuật

- Giai đoạn 10 đạt được cải thiện về sử dụng nhưng chưa xử lý sâu cảnh báo và bảo mật cấu hình.
- Kinh nghiệm: cần tách rõ một giai đoạn hardening chuyên biệt, tránh dàn trải.

---

## 80. Đánh giá kết quả giai đoạn 10

**Đã hoàn thành tốt:**

- Hoàn thiện các điểm điều hướng còn thiếu ở menu `FrmTrangChu` (Thuốc, Dịch vụ, Khách hàng, Admin).
- Tối giản lại cấu trúc menu theo phạm vi chức năng đang có.
- Tinh chỉnh hiển thị `FrmKhachHang` giúp trạng thái UI rõ ràng và thân thiện hơn.
- Duy trì trạng thái build thành công.

**Cần cải thiện tiếp:**

- Ưu tiên xử lý nợ kỹ thuật nullability để giảm cảnh báo và tăng an toàn runtime.
- Tách cấu hình kết nối DB khỏi mã nguồn form để tăng bảo mật và dễ triển khai đa môi trường.
- Rà soát migration rỗng còn tồn tại để làm sạch lịch sử schema.

---

## 81. Kết luận giai đoạn 10

Giai đoạn 10 là bước hoàn thiện tính nhất quán trải nghiệm và điều hướng của hệ thống: các mục menu quan trọng đã được nối đúng luồng xử lý, giao diện khách hàng rõ trạng thái hơn, và dự án vẫn giữ được trạng thái build ổn định. Đây là mốc phù hợp để chuyển trọng tâm sang hardening kỹ thuật toàn diện (nullability, bảo mật cấu hình, chuẩn hóa migration) nhằm đưa hệ thống tiến gần hơn tới mức sẵn sàng vận hành thực tế.
