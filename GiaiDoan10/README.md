...
# README - Giai đoạn 10 (Hoàn thiện thống kê và báo cáo vận hành)

## 74. Tổng quan giai đoạn 10

Giai đoạn 10 tập trung hoàn thiện lớp **thống kê - báo cáo** để hỗ trợ theo dõi tình hình vận hành phòng khám theo thời gian thực.

Trọng tâm chính:

- Bổ sung form `FrmThongKe` cho quản trị và nhân viên.
- Cung cấp chỉ số tổng quan theo khoảng thời gian chọn.
- Trực quan hóa doanh thu 7 ngày gần nhất bằng biểu đồ cột.
- Liên kết điều hướng thống kê từ `FrmTrangChu` theo phân quyền.

---

## 75. Các form và luồng nghiệp vụ nổi bật ở giai đoạn 10

### 75.1. `FrmThongKe` (form mới của giai đoạn 10)

Các chức năng chính đã triển khai:

- Lọc dữ liệu theo `Từ ngày - Đến ngày`.
- Hiển thị 4 chỉ số quan trọng:
  - Doanh thu tạm tính.
  - Số phiếu đã hoàn thành.
  - Số bệnh nhân chờ khám trong ngày.
  - Số lịch hẹn đã hủy trong kỳ lọc.
- Vẽ biểu đồ doanh thu 7 ngày gần nhất bằng `Chart` (WinForms DataVisualization).
- Cung cấp nút đóng form và quay về trang chủ an toàn.

### 75.2. `FrmTrangChu` (bổ sung điều hướng thống kê)

- Bổ sung hàm mở `FrmThongKe`.
- Gắn sự kiện menu `mnuThongKe` để truy cập màn hình báo cáo.
- Kiểm tra quyền truy cập: chỉ `Admin` và `Nhân viên` được phép vào module thống kê.

---

## 76. Công việc kỹ thuật đã thực hiện trong giai đoạn 10

- Kết nối `FrmThongKe` với `PhongMachDbContext` để truy vấn dữ liệu thật từ `HOADON`, `PHIEUKHAM`, `LICHKHAM`.
- Chuẩn hóa bộ lọc thời gian với đầu mút cuối ngày (`AddDays(1).AddTicks(-1)`) để tránh thiếu dữ liệu theo ngày.
- Tính toán tổng doanh thu có xử lý giá trị nullable (`NgayThanhToan`, `Sum decimal?`).
- Hoàn thiện giao diện dashboard dạng card + biểu đồ giúp theo dõi nhanh.
- Bổ sung kiểm tra nhập liệu thời gian (`Ngày bắt đầu <= Ngày kết thúc`).

---

## 77. Giá trị nghiệp vụ đạt được ở giai đoạn 10

- Ban quản lý có màn hình theo dõi hiệu suất hoạt động theo ngày/tuần.
- Nhân viên có công cụ nắm nhanh tình trạng khám và lịch hủy.
- Dữ liệu thanh toán từ các giai đoạn trước đã được tận dụng cho mục tiêu báo cáo vận hành.
- Chuỗi nghiệp vụ của đồ án được mở rộng từ tác nghiệp sang hỗ trợ ra quyết định.

---

## 78. Lỗi sai và rủi ro kỹ thuật có thể gặp ở giai đoạn 10

- Biểu đồ 7 ngày hiện lấy theo `Ngày thanh toán`; nếu dữ liệu thanh toán trễ có thể lệch cảm nhận doanh thu theo ngày khám.
- Chức năng thống kê đang ở mức tổng quan, chưa có drill-down chi tiết theo bác sĩ/chuyên khoa.
- Cần tiếp tục kiểm soát nullability để giảm nguy cơ lỗi runtime khi dữ liệu bất thường.
- Khi build bằng `dotnet build` có thể gặp lỗi `MSB4803` do phụ thuộc `ResolveComReference`; nên build bằng MSBuild/Visual Studio trên Windows.

---

## 79. Khó khăn và kinh nghiệm ở giai đoạn 10

- Khó khăn chính là chuẩn hóa truy vấn thống kê từ nhiều bảng nghiệp vụ khác nhau.
- Kinh nghiệm rút ra: cần thống nhất rõ cột thời gian dùng cho từng KPI (ngày thanh toán, ngày lập phiếu, ngày khám).
- Với dashboard WinForms, nên ưu tiên chỉ số cốt lõi trước, sau đó mở rộng báo cáo chi tiết theo nhu cầu vận hành.
...
- Hoàn thiện module thống kê báo cáo với dữ liệu thực.
- Bổ sung biểu đồ doanh thu trực quan cho 7 ngày gần nhất.
- Kết nối điều hướng và phân quyền từ trang chủ đến module thống kê.

**Cần cải thiện tiếp:**

- Mở rộng báo cáo chi tiết (theo bác sĩ, dịch vụ, khung giờ).
- Chuẩn hóa quy trình build/release để tránh lỗi công cụ build chéo môi trường.

---

## 81. Kết luận giai đoạn 10

Giai đoạn 10 khép lại đồ án bằng việc bổ sung lớp thống kê và báo cáo vận hành, giúp hệ thống không chỉ xử lý nghiệp vụ khám chữa bệnh mà còn hỗ trợ theo dõi hiệu quả hoạt động. Đây là bước hoàn thiện quan trọng để sản phẩm tiến gần hơn tới mức ứng dụng thực tế trong môn học Lập trình quản lý.
