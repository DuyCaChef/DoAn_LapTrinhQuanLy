...
namespace QuanLyPhongMach.Forms
{
    public partial class FrmLichKham : Form
    {
        ...
        private void FrmLichKham_Load(object sender, EventArgs e)
        {
            ...
            rdbNam.Checked = true;
            dtbNgaySinh.MaxDate = new DateTime(2100, 1, 1); // Nới lỏng tạm thời để tránh lỗi ArgumentOutOfRange
            dtbNgaySinh.Value = DateTime.Today;
            dtbNgaySinh.MaxDate = DateTime.Today;

            //Đặt lịch khám từ hôm nay trở đi
            dtbNgayKham.MinDate = new DateTime(1900, 1, 1); // Nới lỏng tạm thời
            dtbNgayKham.Value = DateTime.Today;
            dtbNgayKham.MinDate = DateTime.Today;

            // [ĐÃ SỬA LỖI]: Gỡ sự kiện cũ (nếu Designer đã tự gắn) để tránh đụng độ, sau đó gắn lại an toàn
            dtbNgayKham.ValueChanged -= new EventHandler(dtbNgayKham_ValueChanged);
            dtbNgayKham.ValueChanged += new EventHandler(dtbNgayKham_ValueChanged);

            //Thiết lập combobox giờ khám dựa theo ngày vừa chọn
            LoadDanhSachGioKham();
        ...
        private void LoadDanhSachGioKham()
        {
            ...
            DateTime ngayChon = dtbNgayKham.Value.Date;
        ...
        private void dtbNgayKham_ValueChanged(object sender, EventArgs e)
        {
            LoadDanhSachGioKham(); // Tải lại giờ khám mỗi khi Lễ tân đổi ngày
        }
            ...
            try
            {
                using (var db = new PhongMachDbContext())
                {
                    ...
                    if (taiKhoanTonTai == null)
                    {
                        ...
                        {
                            TenDangNhap = sdt,
                            MatKhauHash = HashPassword(sdt), //Lưu mật khẩu là sđt nhưng hash
                            VaiTro = "KHACHHANG",
                            TrangThai = true
                        };
        ...
        private void btnDatLich_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var db = new PhongMachDbContext())
                {
                    int maBSSave = (int)cboBacSi.SelectedValue;
                    DateTime ngayKhamSave = dtbNgayKham.Value.Date;

                    // =========================================================================
                    // THUẬT TOÁN CHỐNG TRÙNG LỊCH KHÁM 
                    // =========================================================================
                    bool daCoNguoiDat = db.LichKhams.Any(lk =>
                        lk.MaBS == maBSSave &&
                        lk.NgayKham == ngayKhamSave &&
                        lk.GioKham == gioKhamTimeSpan &&
                        lk.TrangThai == "Chờ khám" // Chỉ báo trùng nếu lịch kia chưa khám / chưa hủy
                    );

                    if (daCoNguoiDat)
                    {
                        MessageBox.Show("Bác sĩ này đã có lịch hẹn vào khung giờ này.\nVui lòng thông báo cho bệnh nhân chọn Bác sĩ hoặc khung giờ khác!", "Trùng lịch khám", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return; // Chặn lại, không cho thực hiện lệnh Lưu bên dưới
                    }
                    // =========================================================================
        ...
        private void btnSuaLich_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var db = new PhongMachDbContext())
                {
                    ...
                    // =========================================================================
                    // THUẬT TOÁN CHỐNG TRÙNG (Loại trừ chính lịch đang sửa ra)
                    // =========================================================================
                    bool daCoNguoiDat = db.LichKhams.Any(lk =>
                        lk.MaLichKham != maLichKhamHienTai && // <--- Không báo trùng với chính bản thân nó
                        lk.MaBS == maBSSave &&
                        lk.NgayKham == ngayKhamSave &&
                        lk.GioKham == gioKhamTimeSpan &&
                        lk.TrangThai == "Chờ khám"
                    );

                    if (daCoNguoiDat)
                    {
                        MessageBox.Show("Bác sĩ này đã có lịch hẹn vào khung giờ này.\nVui lòng chọn Bác sĩ hoặc khung giờ khác để đổi lịch!", "Trùng lịch khám", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return; // Chặn lại
                    }
                    // =========================================================================
                    ...
                    if (lkUpdate != null)
                    {
                        lkUpdate.MaBS = maBSSave;
                        lkUpdate.NgayKham = ngayKhamSave;
        ...
        private void btnLamMoi_Click(object sender, EventArgs e)
        {
            ...
            txtSDT.Clear();
            txtDiaChi.Clear();
            rdbNam.Checked = true; // Reset Giới tính về mặc định
            maKhachHangHienTai = 0;
            maLichKhamHienTai = 0; // Đặt lại ID lịch khám = 0 để tránh bấm nhầm nút Xóa

            DateTime thoiGianHienTai = DateTime.Today;

            // [ĐÃ SỬA LỖI NÉM TRẠNG THÁI (OUT OF RANGE)]: Phải nới lỏng Max/Min trước khi gán Value
            dtbNgaySinh.MaxDate = new DateTime(2100, 1, 1);
            dtbNgaySinh.Value = thoiGianHienTai;
            dtbNgaySinh.MaxDate = thoiGianHienTai;

            dtbNgayKham.MinDate = new DateTime(1900, 1, 1);
            dtbNgayKham.Value = thoiGianHienTai;
            dtbNgayKham.MinDate = thoiGianHienTai;
